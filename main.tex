...
Similar reasoning holds for $n = 2$, $m \le 6$: there is a path of length $m + 1 \le 7$, while $2m - 5 \le m + 1 \le 7$, so you cannot do any better than placing nothing. It turns out there is just enough room for the I~piece, and so for $m = 6$ there are two solutions ignoring symmetry:
...
And this reasoning also holds for $\#_{1, n} = n$: the path uses all squares, and since each pentomino costs 5 squares, the best solution is 0 pentominoes.
...
Two squares are \newterm{orthogonal} iff their Manhattan distance is 1. Two squares are \newterm{adjacent} iff their Chebyshev distance is 1 (i.e., they are diagonal or orthogonal). Two sets of squares $S$ and $T$ are \newterm{adjacent} if there is a pair of squares $(s, t) \in S \times T$ (Cartesian product) such that $s$ and $t$ are adjacent. The set of squares adjacent to a set of squares $S$ is denoted $\adj(S)$.
\end{definition}
