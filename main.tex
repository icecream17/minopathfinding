...
And this reasoning also holds for $\#_{1, n} = n$: the path uses all squares, but using at least one pentomino only leaves you with at most one square to work with.
...
A solution is \newterm{brimming} when every square in the grid is used, i.e, every square corresponds to either a pentomino or the path.
\end{definition}
...
We are left with \minordetail{the pentominoes} \pentP{} and \pentT{}. If \pentP{} does not touch the corner, after placing the $2 \times 2$, the 5th square must divide the grid in two. There are four ways to touch a specific corner including symmetry, and two (the horizontal placements) work. For \pentT, there is only one orientation that does not cut the grid. So the three solutions given by \cite[6:04]{v2} are the only ones.
\end{proof}

\begin{theorem}
When a solution is brimming with only one pentomino, for grids with at least 3 columns and 4 rows, for any corner square, the pentomino must include the square diagonally adjacent to that corner square.
\end{theorem}
