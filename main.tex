...
A solution is \newterm{brimming} when every square in the grid is used, i.e, every square corresponds to either a pentomino or the path. In graph theory terms, the path is a Hamiltonian path among the empty squares.
\end{definition}

\begin{lemma}[Brimming solutions in $3 \times n$ grids]
\label{lem:brim3xn}

Say we know how many pentominoes are in a solution, and that all squares are used. Then we know any solution must be brimming, and so:
\begin{itemize}
    \item \pentF, \pentX, and \pentU{} are not part of the solution.
    \item \pentL, \pentN, and \pentY{} must have their $1 \times 4$ in the middle row, if none of the columns the pentomino is in contains any part of another pentomino.
\end{itemize}
\end{lemma}

\begin{proof}
First, we take \pentU{} from the first bullet point. If a path enters \minordetail{or exits} a cave square \minordetail{(enters without loss of generality)}, it cannot exit since there is only one entrance. Since \pentU{} has a cave square, we can say a path must end there. Placing \pentU{} vertically is eliminated by \cref{cor:cutcorner3xn}. But if \pentU{} is placed horizontally (2 rows by 3 columns, \cref{fig:u3x4}), it is at least as good to just go around the \pentU{} (removing any obstacles).
\begin{itemize}
...
\footnotetext{film2860 on discord used gpt-4o to try to remove pixel artifacts. It generated equivalent code, which did not fix the problem, but did illustrate the usage of scopes which was helpful as a beginner. So congrats on contributing!}% \marginnote did not fit
...
When a solution is brimming with only one pentomino, for grids with at least 3 columns and 4 rows, the pentomino must include all squares diagonally adjacent to a corner square of any ($3 \times 4$)-subgrid.
\end{theorem}
...
\begin{figure}
    ...
    \begin{tikzpicture}
        \draw[step=1cm,black,very thin] (0,0) grid (4,3);
        \filldraw[white, postaction={pattern={Lines[angle=45, distance=2mm, line width=1mm]},pattern color=gray}] (1,1) rectangle (2,2);
...
By \cref{fig:3x4}, we have brimming solutions with 1 pentomino, and since $3 + 4 - 1, 12 - 10 < 7$, \cref{prop:oneplace} and therefore \cref{lem:brim3xn} applies. \pentL, \pentN, and \pentY{} can be eliminated since a $1 \times 4$ in the center row would divide the grid into two subgrids (this is not allowed; remember that every nonempty square must be in the path).

We are left with \minordetail{the pentominoes} \pentP{} and \pentT{}. By \cref{th:brimming1}, they must include the center two squares. All orientations of \pentP{} and \pentT{} with this restriction works, and so the three solutions given by \cite[6:04]{v2} are the only ones.
\end{proof}
...
\begin{proof}
We have a similar setup to \cref{th:3x4}, but with the examples from \cref{fig:3x5}, $3 + 5 - 1, 15 - 10 < 10$, and still \cref{prop:oneplace} and \cref{lem:brim3xn}; and by \cref{th:brimming1} all solutions must include the center three squares. If the two additional squares are not orthogonally adjacent, it would split the grid into two, so the three solutions given by \cite[6:10]{v2} are the only ones.
\end{proof}
