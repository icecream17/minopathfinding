...
%A platter's \newterm{caves} are made of the squares of its border that are not in its bound. $\caves(P) = \border(P) \setminus \bound(P)$
%\end{definition}
%
...
%        \draw (2.5,2.5) node{$C$};
%    \end{tikzpicture}
%    \caption{U's border squares and sole cave square are marked $B$ and $C$ respectively.}
%    \label{fig:ucave}
%\end{figure}
%
...
%\end{definition}
%
%\begin{corollary}[Convex bound]
%\label{cor:Convex bound}
%
%Lemma \ref{lem:Rectangle cut} extends to any shape, so long as the following statement holds: "the shortest path between any two squares along the border, if it exists, cannot get shorter upon removing the platter". This could be a similar construction to the rectangle blockage, but it extends more generally to include, for example, some placements of pentominoes at the corner.
%\end{corollary}
