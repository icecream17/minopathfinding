...
\begin{proof}
By Figure~\ref{fig:3x4}, we have maximal solutions with 1 pentomino, and since $3 + 4 - 1, 12 - 10 < 7$, Lemma~\ref{lem:max3x3} applies. L, N, and Y can be eliminated since a $1 \times 4$ in the center row would divide the grid into two subgrids (this is not allowed; remember that every nonempty square must be in the path).
...
A polynomial's \badterm{contribution} is informally how much longer it makes a given path. But Figure~\ref{fig:nontrivialcontribution} shows a formal definition is still a ways away. In such examples, we must consider entire platters.
